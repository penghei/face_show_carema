  ...
  useLayoutEffect(() => {
    let sub = PubSub.subscribe("playedTime", (_, data) => {
      ...
      if (foundIndex !== -1) {
        ...
        let foundIndexInc =
          foundIndex + 6 > allLyric.length ? allLyric.length : foundIndex + 6;
        if (foundIndex <= 5) {
  ...
  return (
    <div className="lyricsMain">
      ...
      {partLyric.length ? (
        ...
        <h2 className="lyric-loading">
          歌词正在加载哦~或许根本没有歌词也说不定！
        </h2>
      )}
    </div>
  );
...
const MusicLyricsUI = connect((state) => ({
