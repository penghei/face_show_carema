  ...
  return (
    <div className="face-show-detail">
      <div className="face-show-detail-content">
        <div>
          您当前的情绪：
          <Tag color={userMsgObj.emotion ? "green" : "orange"}>
            {emoDict[userMsgObj.emotion] || "暂未识别"}
          </Tag>
        </div>
        <div>
          推荐播放列表：
          <Tag color={userMsgObj.emotion ? "magenta" : "orange"}>
            {userMsgObj.emotion ? "推荐成功" : "暂未识别"}
          </Tag>
        </div>
        <div>
          当前播放歌曲：
          <Tag color={userMsgObj.musicName ? "purple" : "orange"}>
            {userMsgObj.musicName || "暂未识别"}
          </Tag>
        </div>
      </div>
    </div>
