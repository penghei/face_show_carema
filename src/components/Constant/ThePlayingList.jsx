...
function ThePlayingList(props) {
  ...
  useEffect(() => {
    Pubsub.subscribe("selectedListId", (_, data) => {
      if (data) {
        axios
          .get(`/apc/playlist/detail?id=${Number(data)}`)
          .then((res) => {
            let allSongs = [];
  ...
  function getAllSongs(allSongs) {
    //这是每一首个的id组成的数组,可用来请求详情
    let allSongsId = [];
    allSongs.forEach((obj) => {
      allSongsId.push(obj.id.toString());
    });
    axios.get(`/apc/song/url?id=${allSongsId.toString()}`).then((res) => {
      let newSongs = [];
      res.data.data.forEach((songUrlObj) => {
        let foundSong = allSongs.find((songDetailObj) => {
          //在歌曲详情中查找歌曲名等信息
  ...
  return (
    <div>
      <span className="drawer-btn" onClick={showDrawer}>
        <svg
          t="1635407212844"
...
const ThePlayingListUI = connect(
  ...
  (dispatch) => ({
    setPlayingSong: (value) =>
      dispatch({ type: "setPlayingSong", data: value }),
    emptyPlayingList: () => dispatch({ type: "emptyPlayingList", data: [] }),
    addToPlayingList: (value) =>
      dispatch({ type: "addToPlayingList", data: value }),
    setNewPlayingList: (value) =>
      dispatch({ type: "setNewPlayingList", data: value }),
  })
)(ThePlayingList);
